...
const LANG_GAMES = ["orthographe", "dictee", "bonne-phrase"];
...
function renderProgressChart(scores) {
  ...
  const data = scores.map((r) => Math.round((r.score / r.total) * 100));
...
function renderStrengthsChart(scores) {
  ...
  scores.forEach((r) => buckets[categorizeScore(r)].push((r.score / r.total) * 100));
