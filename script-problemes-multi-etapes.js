...
function validate() {
  ...
  if (isCorrect) {
    score++;
    if (window.clearMistake) clearMistake("problemes-multi-etapes", data.text);
  } else {
    if (window.recordMistake) recordMistake("problemes-multi-etapes", difficulty, data.text, data);
  }
