...
Site statique (HTML / CSS / JS) avec comptes utilisateurs, sauvegarde des scores et page de statistiques via [Supabase](https://supabase.com).
...
**Si tu avais déjà déployé le site avant l'ajout de la page statistiques**, exécute en plus `add-lang-column.sql` dans le SQL Editor de Supabase (ajoute la colonne qui permet de savoir si chaque partie a été jouée en français ou en anglais — nécessaire pour le graphique radar).
...
- **Évolution des scores** — courbe du score (%) partie après partie, toutes langues et jeux confondus
- **Points forts** — graphique radar à 3 axes : Français / Anglais / Mathématiques, basé sur la moyenne des scores de chaque catégorie
...
add-lang-column.sql            → migration à exécuter si ta base existait déjà avant cette mise à jour
```
